...
local bfsDir = {'singLEFT','singDOWN','singUP','singRIGHT','singUP','singLEFT','singDOWN','singUP','singRIGHT'}
...
local presses = {'A press','B press','C press','D press','E press','F press','G press','H press','I press'};
local confirms = {'A confirm','B confirm','C confirm','D confirm','E confirm','F confirm','G confirm','H confirm','I confirm'};
...
function judgeNote(diff) --sorry i steal code bb
	...
	for i=0, #windowNames-1 do --noticed i couldnt get shits, so i fixed it lol
		if diff <= timingWindows[round(math.min(i+1, #windowNames))] then
		
			return windowNames[i+1];
		end
	end
	return 'shit';
end
